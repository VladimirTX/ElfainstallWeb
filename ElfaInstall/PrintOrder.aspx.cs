    ...
    public partial class PrintOrder : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object Sender, EventArgs E)
        {
            ...
            if (_drOrder.HasRows)
            {
                ...
                if (decimal.Parse(_drOrder["MilesPrice"].ToString()) > 0)
                { lblMilesPrice.Text = decimal.Parse(_drOrder["MilesPrice"].ToString()).ToString("c"); }
                else { lblMilesPrice.Text = "$ __________"; }
                if (decimal.Parse(_drOrder["DemoPrice"].ToString()) > 0)
                { lblDemoPrice.Text = decimal.Parse(_drOrder["DemoPrice"].ToString()).ToString("c"); }
                else { lblDemoPrice.Text = "$ __________"; }
                if (decimal.Parse(_drOrder["MiscPrice"].ToString()) > 0)
                { lblMiscPrice.Text = decimal.Parse(_drOrder["MiscPrice"].ToString()).ToString("c"); }
                else { lblMiscPrice.Text = "$ __________"; }
                if (decimal.Parse(_drOrder["TipPrice"].ToString()) > 0)
                { lblOther.Text = decimal.Parse(_drOrder["TipPrice"].ToString()).ToString("c"); }
                else { lblOther.Text = "$ __________"; }

                if (decimal.Parse(_drOrder["PromoPrice"].ToString()) > 0)
                { lblDiscount.Text = (-1 * decimal.Parse(_drOrder["PromoPrice"].ToString())).ToString("c"); }
                else { lblDiscount.Text = "$ __________"; }
                ...
                string options = _drOrder["Options"].ToString();
                if (options.Trim() != "" && options.IndexOf("ATH") != -1) lblHeader.Text = "AT<b>HOME</b> Invoice";
            }
            _drOrder.Close();
        }
        decimal GetTaxRate()
